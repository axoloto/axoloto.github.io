...
Well guess what, boids simulation being fully functional, now is the time for the real game: **implementing a fluids simulation**! Let's begin.
...
Before diving into numerical schemes and solvers, first we need to choose between two ways of looking at fluids motion:
...
2. **Lagrangian description** - **We look at the fluids as a group of individual particles, analyzing their interactions to determine their trajectories and conditions.** Contrary to the Eulerian approach, the domain is not considered as a whole and matters less. Hence, no need of mesh or grid, simply a bunch of particles on which we apply some rules modelizing the fluids interaction. To make it simple, **it is a bit like approximating a fluids domain with a ball pit**. Historically, Lagrangian simulations have been considered less precise than Eulerian ones. Their biggest force is their execution speed, they **can be used for coarse but fast and visually acceptable simulations**. Thus, they are more employed in the world of **computer graphics** than in CFD. By being meshfree, it is also well suited for free-surface problems or ones with large boundary displacements.
...
*Exemple of smoothing kernel functions used in our model and presented in [Muller et al., 2001](https://matthias-research.github.io/pages/publications/sca03.pdf)*
