...
Creating an empty C++ desktop application from scratch can be very laborious. You need to connect all your dependencies: different codebases, third-parties that you need to build or get the right binaries, scripts for the environment, scripts for the build system... Everything must be at the right place with the right connection and the good options. Build it once, fail, fix it, build it twice, fail and try again. Concerning the build, if you want something to be at least a bit portable, you need to use CMake. You also really want to add a version control system, Git for example. Besides, it is better to add some formatting tools like clang-format to have something clean. In addition, to have an application with its own cross-platform window, you need to use a specific third-party like SDL. If including OpenGL or OpenCL, you need to include specific APIs to connect to your GPU. At that time, one of my friends introduced me to Conan and I happily embraced it. Conan is a powerful c++ package manager which help you include third-party libraries already built on remote servers by users or even to build them locally. Conan will automatically generate a cmake file pointing to the different generated or retrieved binaries, making the third-parties integration significantly easier.

Overall, that is a lot of things to grasp and understand. It took me literally one weekend to figure this out, choosing the right third-parties, wandering through hollow error messages, stupid mistakes and finally making the whole thing consistent. All this installation step is definitely not straightforward and from my current perspective, after being done with all of that, I consider it as one of the hardest steps of the whole project. I think it is probably quite common among c++ developers to struggle with this initialization stage, because we usually work on big projects where someone else took care of setting up everything for us. How privileged we are.

*The great black window*

Okay, I have an empty window running on Windows. Great. Now I want to create a box in 3D and be able to move around. That's another fun bit, setting up a 3D rendered from scratch. First of all, you need a GUI in which you will incorporate a 3D viewport. I chose Dear ImGui which is a light-weight, open source, friendly immediate-mode graphics user interface. See it as a fast way to prototype stuff, far away from more complete but heavy retained-mode Guis like Qt. At each frame, Dear ImGui will regenerate everything, there is no persistent storage of information. It is also directly incorporated in the rendering loop. All of this makes it easy to prototype and debug. Besides, it can be directly connected to SDL, and OpenGL by using Glad, another third-party library! Once ImGui (let's forget the Dear) is in place, you can easily create some widgets, connect the mouse and the keyboards with SDL and have something interactive and responsive. Once this is done, the real work on the renderer begins with the implementation of shaders, camera, connection to mouse and keyboard. Using the ImGui's OpenGL context, we can generate something in a 3D world. Drawing a box is pretty straightforward at that point. I added a bunch of points in it, with random positions and colors, those are my future particles on which I will apply the boids rules. After days of tweaking stuff, something is finally moving! 

*Box with particles* 

## First CPU boids draft

Particles are finally here! Note that the renderer is only meant to display things and nothing else, it has no clue about physics. To make the particles behaving like boids, we need a physics engine which computes and updates coordinates of each particle at every frame. I really want to have a clean and clear codebase with separated submodules, to help with debugging and potential futur development. Therefore, interactions between graphics and physics engines are limited as much as possible.
...
As the weighted combination of these three rules can give a variety of different boids behaviors, their weights are available in the UI. On top of it, I also implemented another optional rule to add an attractive/repulsive target, similar to adding a boids leader or a predator. 
